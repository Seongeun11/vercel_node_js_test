...
export default function AdminUsersPage() {
  ...
  const handleChange = (key: keyof CreateUserForm, value: string) => {
    setForm((prev) => ({
      ...prev,
      [key]: value,
    }))
  ...
  const validateForm = () => {
    ...
    if (!/^\d+$/.test(form.student_id.trim())) return '학번은 숫자만 입력해주세요.'
    if (!form.password.trim()) return '비밀번호를 입력해주세요.'
    if (form.password.trim().length < 6) return '비밀번호는 최소 6자 이상이어야 합니다.'
    return ''
  }
