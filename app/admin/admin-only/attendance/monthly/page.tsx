...
export default function MonthlyAttendancePage() {
  ...
  /**
   * 월별 출석 데이터는 검색 조건이 바뀔 때마다 다시 조회
    ...
    <main style={{ padding: '24px' }}>
      <section style={{ marginBottom: '24px' }}>
        ...
        <p style={{ marginTop: '8px', color: '#666' }}>
          관리자 전용 읽기 화면입니다. 출석 수정과 엑셀 다운로드는 다음 단계에서 추가합니다.
        </p>
      </section>
          ...
          >
            <table
              style={{
                ...
                minWidth: Math.max(900, 320 + data.occurrences.length * 96),
                fontSize: '14px',
              }}
            >
              <thead>
                <tr style={{ background: '#f9fafb' }}>
                  <Th sticky left={0} width={110}>
                    학번
                  </Th>
                  <Th sticky left={110} width={110}>
                    이름
                  </Th>
                  <Th width={80}>기수</Th>

                  {data.occurrences.map((occurrence) => (
                    <Th key={occurrence.id} width={110}>
                      <div>{occurrence.occurrence_date.slice(5)}</div>
              ...
              <tbody>
                {data.rows.length === 0 ? (
                  <tr>
                    <td
                      colSpan={3 + data.occurrences.length}
                      style={{
                        textAlign: 'center',
                        padding: '32px',
                        color: '#6b7280',
                  ...
                  data.rows.map((row) => (
                    <tr key={row.profile_id}>
                      <Td sticky left={0} width={110}>
                        {row.student_id}
                      </Td>
                      <Td sticky left={110} width={110}>
                        {row.full_name}
                      </Td>
                      ...
                      {data.occurrences.map((occurrence) => {
                        ...
                        return (
                          <Td key={occurrence.id} width={110} center>
                            <span
